...
Utility_Leviathan <- function(data1, var, myfactor, normality, shape_palette, fill_palette, switch, panel, scalefactor, scalefactorlabel){
  ...
  filtered_words_edit <- gsub("pos", "", fixed = TRUE, filtered_words)

  Attempt <- data.frame(Fluorophore = filtered_words_edit)
  Attempt2 <- left_join(Attempt, panel, by = "Fluorophore")
  TheWords <- Attempt2$Marker
  TheWords <- gsub("(\\b\\w+\\b)", "\\1+", TheWords)
  cleaned_string <- paste(TheWords, collapse = " ")
