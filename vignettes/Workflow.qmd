...
UpdatedTemplatePath <- list.files(folder, patten="UpdatedTemplateForGates.csv", full.names=TRUE)

UpdatedTemplate <- read.csv(TheTemplateCSV, UpdatedTemplatePath, check.names=FALSE)
```
...
TheGateCutoffs <- purrr::map(.x=UnmixedGatingSet, .f=Coereba_GateCutoffs,
  subset="root", sample.name="GROUPNAME", desiredCols=MarkersForToday,
  GatingTemplate=UpdatedTemplate, GateOverwrite=TRUE) %>% bind_rows()

#SecondGates <- file.path("template", "SecondGates.csv")
#write.csv(TheGateCutoffs, SecondGates, row.names=FALSE)
#UpdatedGates <- read.csv(SecondGates, check.names=FALSE)
#UpdatedGates <- TheGateCutoffs
```
