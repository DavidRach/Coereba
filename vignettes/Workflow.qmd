...
Having gone through and corrected the splitpoints, it is now time to update the SecondGates.csv to reflect the changes. To facilitate this, we will use the Coereba_UpdateGates function.
...
With the gate locations now determined, it's time to run Coereba and classify each individual cell within  a specimen by it's marker expression. We will use Utility_Coereba for this process. 
...
We can optionally save this output to an .fcs file. This can allow for later referece, as well as permit us to run dimensionality visualization algorithms, gate the islands, and then retrieve Coereba data for the respective islands. 

Likewise, we can retrieve the same data back to R through the Coereba_FCS_Reversal function. 

# Generating a Summarized Experiment File

For long-term handling, we can save the Coereba output, alongside metadata and panel information, into a Summarized Experiment file. This will permit us to use a lot of the existing Bioconductor package infrastructure for analysis. We do this through the Coereba_Processing function. 

# Retrieving Marker Expressions

From the SE object, we are working with the individual Coereba cluster counts. For some analyses, we may want to know what proportion of cells for a particular donor or island expressed a particular marker, so we may not want as fine-grained separation in clusters. We can reaggregate the populations present and retrieve the proportions in this manner. 
