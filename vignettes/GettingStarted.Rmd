...
# Utility_MarkerExpressions

From the same Coereba identities, we can backcalculate the proportion of cells for a given cell node of interest that express any given marker (see ILT paper). The Utility_MarkerExpressions() function was the original implementation of this idea. See 5-subject notebook remodel post submission. 
...
An initial attempt to deal with the single outlier dot issue from Utility_Behemoth by dropping a top value from all the groups. Not likely being kept current state given what we have learned since. 
