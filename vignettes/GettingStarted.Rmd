...
abstract: |
  When dealing with cytometry data, how many clusters are present is often a question users ask. The answer is always it depends, but having ways to visualize this decision making process would be helpful. Coereba is an R package that attempts to provide some tools to qualitatively and quantitatively assess the variation present, and allow for flexibility to deal with datasets that may have batch effects that limit their use for unsupervised analytical approaches highly dependent on consistent MFI acrosss individual experiments and files.
...
### Adjusting the Gate Cutoffs with Coereba Shiny App

```{r}

```

### Updating GateCutoff CSV and Final Checks

```{r}

```

### Running Coereba on a GatingSet.
